...
export function ChatInterface({
  ...
  return (
    <div className="flex min-h-[620px] flex-col gap-4">
      <div className="glass-panel flex-1 overflow-hidden border border-border/70 p-0">
          ...
          <div className="space-y-4">
              ...
              >
                <div
                  className={cn(
                    ...
                    message.role === 'user'
                      ? 'border-transparent bg-[rgb(16,111,99)]/0.9 text-white'
            ...
            {pendingMessages.map((msg, index) => (
              <div key={`pending-${index}`} className="flex justify-end">
                <div className="max-w-[75%] rounded-3xl border border-transparent bg-[rgb(16,111,99)]/0.5 px-4 py-3 text-sm text-white">
                  <div className="prose prose-sm max-w-none dark:prose-invert">
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="rounded-3xl border border-border/60 px-4 py-2">
                  <Loader2 className="h-5 w-5 animate-spin text-[rgb(16,111,99)]" />
                </div>
              </div>
            )}
      ...
      <form onSubmit={handleSubmit} className="glass-panel border border-border/70 px-4 py-4">
        <div className="flex flex-col gap-3 md:flex-row">
          <Textarea
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder="Digite sua mensagem..."
            className="min-h-[72px] flex-1 resize-none rounded-2xl border-border/80"
            onKeyDown={(e) => {
